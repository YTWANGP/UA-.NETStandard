...
COPY . ./
WORKDIR /app/ComIOP/ConsoleWrapper
RUN dotnet restore

# copy and publish app
#WORKDIR /app
#COPY ConsoleWrapper/. ./ConsoleWrapper/
#WORKDIR /app/ConsoleWrapper
RUN dotnet publish -c Release -o out

# start it up
FROM microsoft/dotnet:${runtime_base_tag} AS runtime
WORKDIR /app
COPY --from=build /app/ComIOP/ConsoleWrapper/out ./
WORKDIR /appdata
ENTRYPOINT ["dotnet", "/app/UA COM Console Server Wrapper.dll"]