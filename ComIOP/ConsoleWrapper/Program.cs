...
namespace Opc.Ua.Com.Client
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("OPC UA Console Server Wrapper");
    ...
    public class MySampleServer
    {
        ...
        public void Run()
        {
            ...
            if (server != null)
            {
                ...
                using (ComWrapperServer _server = server)
                {
                    // Stop status thread
                    server = null;
                    status.Wait();
        ...
        private async Task ConsoleSampleServer()
        {
            ...
            // start the server.
            server = new ComWrapperServer();
            await application.Start(server);

            foreach (EndpointDescription endpoint in server.GetEndpoints())
            {
                Console.WriteLine("Server Endpoints URLs: " + endpoint.EndpointUrl);
            }
        ...
        void PrintSubscriptionStatus(Subscription subscription, string reason)
        {
            string item = String.Format("{0,9}:", reason);
            item += String.Format("SubscriptionId " + subscription.Id.ToString());
            item += String.Format(" :PublishingInterval " + (int)subscription.PublishingInterval);
            item += String.Format(" :MonitoredItemCount " + subscription.MonitoredItemCount);
            lock (subscription.DiagnosticsLock)
            {
                item += String.Format(" :SeqNo " + subscription.Diagnostics.NextSequenceNumber);
            }
            Console.WriteLine(item);
        }
        ...
        private async void StatusThread()
        {
            while (server != null)
            {
                if (DateTime.UtcNow - lastEventTime > TimeSpan.FromMilliseconds(6000))
                {
                    ...
                    IList<Subscription> subscriptions = server.CurrentInstance.SubscriptionManager.GetSubscriptions();
